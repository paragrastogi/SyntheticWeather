...
path_fldr_pickle_def = os.path.join('/home', 'rasto', 'Documents',
                                    'WeatherData',
                                    'SyntheticData', 'pickled')


def synweather(stcode='GLA', path_wthr_fldr='/usr/esru/esp-r/climate',
               outpath=os.getcwd(),
               l_start=int(0), l_end=int(31*24),
               l_step=int(4*24), histlim=int(14*24),
               stlat=0.0, stlong=0.0, stalt=0.0,
               path_fldr_pickle=path_fldr_pickle_def):

    #    # List files in weather folder.
    #    list_wfiles = os.listdir(path_wthr_fldr)
    #    # Find the file that contains the station code.
    #    wfile_name = [file for file in list_wfiles if stcode in file]

    #    # Path to the actual weather file.
    #    path_wthr_file = os.path.join(path_wthr_fldr, wfile_name)
    ...
    if (currentdata is None):
        ...
        # Longitude.
        stlong = citytab.Longitude[41]
        # Latitude.
        stlat = citytab.Latitude[41]
        # Altitude.
        stalt = citytab.Altitude[41]
    ...
    # Change the month_tracker[1]==month_trackervalue of variables *l_start* and *l_end* to alter
...
#    l_step = int(4*24)
    ...
    for h in range(l_start+l_step, l_end+l_step, l_step):
        ...
        month_tracker[d] = np.squeeze(currentdata.month.values[h == hh])
    ...
    # for c, colname in enumerate(column_names):
    #    gp_list[c][0:int((l_start+l_step)/24)] = []
