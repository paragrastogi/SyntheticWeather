...
While the concept of a generator and the method used here are both based on
earlier work published by me (Parag) in 2015 and 2016, the method used
here is substantially different from the previous generator.
...
currentdata, historicaldata = gw.get_weather(stcode, citytab)

# For now, I am treating the TMY file as "real-time measurements" or forecasts.
# In "prediction mode" the actual forecast or measurement would be
# used instead. Essentially, the model is "learning" on historical data,
# but "predicting" on 'current' or recent data.
# In "generation" mode, the model is doing the same, but the current data
# can be replaced with one-time-series-at-a-time from the historical data.

# T = currentdata.shape[0]
# TT = historicaldata.shape[0]
...
hist_len = ts_hist_in.shape[0]
# Number of met variables being considered =
# number of columns minus time variables.
metvars = ts_curr_in.shape[1] - 2
...
# scaled_days = ts_hist_norm[:, 0]
# scaled_hours = ts_curr_norm[:, 1]
...
for h in range(l_start+l_step, l_end+l_step, l_step):
    ...
    for c, colname in enumerate(column_names):
        ...
        if colname in date_cols:
            ...
            x_pred = np.squeeze(gp_temp.sample_y(
                    x_query, n_samples=n_samples), axis=1)
...
for c, colname in enumerate(column_names):
    ...
    if colname in ['ghi', 'dni', 'dhi']:
        ...
        for n in range(0, n_samples):
            ...
            nans = np.isnan(temp)
            temp[nans] = np.interp(
                    ts_curr_in[nans, 1], ts_curr_in[~nans, 1], temp[~nans])
            xout_un[:, c, n] = temp
...
# This is mostly only useful for applications where forecasts are needed,
# e.g., controls.
rmse_pred = np.zeros([ts_curr_in.shape[1], n_samples])
cv_rmse = np.zeros([ts_curr_in.shape[1], n_samples])

for c, colname in enumerate(column_names):

    if colname not in date_cols:

        for n in range(0, n_samples):

            rmse_pred[c, n] = rmseloss(ts_curr_in[:, c]-xout_un[:, c, n])
            cv_rmse[c, n] = rmse_pred[c, n]/np.average(ts_curr_in[:, c])

rmse_avg = np.average(rmse_pred, axis=1)
cv_rmse_avg = np.average(cv_rmse, axis=1)
...
p1 = plt.scatter(ts_curr_in[plotrange, 2], ts_curr_in[plotrange, 3],
                 color=colours.blackest)
p1 = plt.scatter(xout_un[plotrange, 2, n], xout_un[plotrange, 3, n],
...
# Repeat the RGB spec for grey n_sample times to plot n_sample
# synthetic series.
grey_reps = np.repeat(np.atleast_2d(colours.grey).T, n_samples, axis=1).T

# Histogram.
ax = plt.figure(num=None, figsize=(6, 4), dpi=80,
                facecolor='w', edgecolor='k').add_subplot(111)
