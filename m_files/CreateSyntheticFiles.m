...
hrmdlfile = p.Results.hrmdlfile;
fourierfile = p.Results.fourierfile;
...
lastdim = size(FutureWeather.TDBdmean.rcp45, 3);

for p = 1:length(UniqueParams)
	FutureWeather.(UniqueParams{p}) = ...
		structfun(@(x) reshape( ...
		permute(x, [2 1 3]), [], lastdim), ...
		FutureWeather.(UniqueParams{p}), ...
		'UniformOutput',0);
end
...
nanfinder = sum(isnan(FutureWeather.TDBdmean.rcp45)) + sum(isnan(FutureWeather.TDBdmean.rcp85)); 
PickModel = find(nanfinder == min(nanfinder));
if length(PickModel)>1
...
% % This is a bunch of expensive-to-calculate summary statistics that you
% % % can enable if really interested.
...
save(fullfile(path_save_fldr, [nameEPWfile,'_Syn.mat']), ...
	'syndata', '-v7.3')
...
% % Also disabled for quicker generation of synthetic files.
% Un-comment if you want to calculate coverage of various quantiles.
